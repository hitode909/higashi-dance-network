var Gallery, Page, YoutubeLooper, YoutubeSearcher, bindLoadVideoFormEvents, bindLoopControllerEvents, setTweetLink, setup, setupPopState, setupSaveLoop, setupSearch, setupSelectOnClick, setupToggle, setupUrlShare;
Page = {
...
setupToggle = function() {
  return $(document.body).on('click', '.toggle', function() {
    var id;
    id = $(this).attr('data-for');
    return $(document.body).find('#' + id).toggle();
  });
};
setupSaveLoop = function() {
  return $('#save-loop').click(function() {
    ...
    return Gallery.post(YoutubeLooper.playingVideoId, YoutubeLooper.seekToPosition, YoutubeLooper.seekDuration).then(function() {
      return Gallery.setupRecentLoops();
    }).always(function() {
